...
SOURCE_PATH = 'abfss://scgpdldev@scgpkgdldevhot.dfs.core.windows.net/EDW_DATA_LANDING/scgp_fi_acct/data_p/'
...
# MAGIC %md
# MAGIC # 🎃 Extract data from the excel Sheet: P2022
...
try: 
      ...
      df_a = pd.read_excel(files.path.replace('dbfs:', ''), sheet_name=f"P{year}", header=None, dtype=data_types)
...
psdf_a.createOrReplaceTempView("tmp_source_datap")
...
# MAGIC tmp_excel_cad_datap_consolidate_data_upd
# MAGIC AS
# MAGIC SELECT * FROM tmp_source_datap A
# MAGIC WHERE NOT EXISTS (
# MAGIC     SELECT 1 FROM scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_consolidate_data B 
...
# MAGIC %sql 
# MAGIC MERGE INTO scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_consolidate_data T1
# MAGIC USING tmp_excel_cad_datap_consolidate_data_upd T2
...
# MAGIC %sql  
# MAGIC INSERT INTO scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_consolidate_data (Line_No, Dimension, Year, Month, Amount, Account, scd_active, scd_start, scd_end, src_hash_key, src_hash_diff)
# MAGIC SELECT Line_No, Dimension, Year, Month, Amount, Account, scd_active, scd_start, scd_end, src_hash_key, src_hash_diff
# MAGIC FROM tmp_excel_cad_datap_consolidate_data_upd
...
try: 
      ...
      # Loop get all file in the folder
      df_cost = pd.read_excel(files.path.replace('dbfs:', ''), sheet_name=f"CostP{year}", header=None, dtype=data_types)
...
psdf_cost.createOrReplaceTempView("tmp_source_datap_cost")
...
# MAGIC %sql
# MAGIC CREATE OR REPLACE TEMPORARY VIEW 
# MAGIC tmp_excel_cad_datap_consolidate_cost_data_upd
# MAGIC AS
# MAGIC SELECT * FROM tmp_source_datap_cost A
# MAGIC WHERE NOT EXISTS (
# MAGIC     SELECT 1 FROM scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_costa_consolidate_data B 
# MAGIC     WHERE A.src_hash_key = B.src_hash_key 
# MAGIC     AND  A.src_hash_diff = B.src_hash_diff 
# MAGIC     AND B.scd_active = 1
# MAGIC )
...
# MAGIC %sql 
# MAGIC MERGE INTO scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_costa_consolidate_data T1
# MAGIC USING tmp_excel_cad_datap_consolidate_cost_data_upd T2
# MAGIC ON T1.src_hash_key = T2.src_hash_key AND T1.scd_active = 1
# MAGIC WHEN MATCHED THEN UPDATE SET T1.scd_active = 0 , T1.scd_end = CURRENT_TIMESTAMP()
...
# MAGIC %sql  
# MAGIC INSERT INTO scgp_edl_dev_uat.dev_scgp_edl_staging.excel_cad_datap_costa_consolidate_data (Line_No, Dimension, Items, Unit, Year, Month, Amount, scd_active, scd_start, scd_end, src_hash_key, src_hash_diff)
# MAGIC SELECT Line_No, Dimension, Items, Unit, Year, Month, Amount, scd_active, scd_start, scd_end, src_hash_key, src_hash_diff
# MAGIC FROM tmp_excel_cad_datap_consolidate_cost_data_upd
